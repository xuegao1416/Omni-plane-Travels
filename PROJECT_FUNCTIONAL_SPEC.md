# Omni-plane Travels（世界漫游指南）功能总图（v2.8.4 当前代码基线）
...
3. **数据格式兼容窗口只保留直接上一代。** 例如当前 schema V3 可保留 V2→V3；V1→V2、V0→V1 之类更早链路应从当前运行时移除。数据库部署版本独立判断：IndexedDB 必须支持 DB v4～v9 直接补齐到当前 v10（包括已有消息分片的 v4 和 2.8.2 的 v5），不改写存档记录；数据库 SQL 迁移历史也不按此规则删除。
