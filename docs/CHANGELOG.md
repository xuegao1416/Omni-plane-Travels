# 更新日志

## v2.8.4 — 玩法模块共创 Agent、数据库 v4 升级与设置页性能修复 (2026-09-19)

本版本合并 v2.8.3 之后的全部开发提交，并收尾数据库升级窗口、剧情导演降级提示与设置页性能。

### ✨ 玩法模块工坊（共创 Agent）

- 工坊升级为原生工具调用共创 Agent：通过能力查询、草稿创建与局部修改、校验和真实试玩共同制作玩法模块；聊天、执行记录和模块版本分别管理。
- 模块可原子消耗或增加金币、背包物品和生存资源；资源不足、重复事件或规则失败不会产生部分结算。
- 支持版本差异、显式回退与绑定世界；对既有存档提供预览影响、手动应用和恢复点还原，旧 V2 会话首次打开对应世界时迁移。

### 🔧 修复与优化

- **共创兼容性**：不支持原生工具调用时自动尝试文本操作，严格校验后才修改草稿；兼容接口拒绝可选参数时有界降级，修复重复工具 ID 和末步完成误报。
- **小说与导演补强**：保留分段未变化的分析检查点，导入资料先校验再恢复主线依赖；自动人物观察只同步正文中的在场角色，阻止空阶段和依赖后续阶段导致的主线卡住。
- **请求与导入边界**：变量提取的超时和取消覆盖响应正文读取；结构化请求可降级普通文本，拒绝截断、过滤和取消后的结果；修复导入模块期间切换会话可能覆盖其他草稿的问题，并拦截保留字模块 ID。
- **数据库升级**：IndexedDB 支持 v4～v9 无损补齐到 v10，修复旧用户导入 TXT 被版本门槛拦截的问题；早于 v4 的数据库仍停止升级并保留原数据。
- **请求稳定性**：修复兼容接口鉴权头、短流式回答被误判为 429 降级、工具 Schema，以及 ComfyUI 的类型收窄问题。
- **剧情导演**：结构协议连续失败时，降级提示重新上报给用户，不再静默。
- **设置页性能**：设置界面打开后暂停被遮挡页面的动画并移除大面积双层模糊，降低浏览器持续渲染负担。
- **界面细节**：完善人物迷雾、物品展示、世界大厅和玩法模块界面，继续优化移动端布局。

### ✅ 验收记录

- 类型检查通过；相关模块、API、发布、数据库与导演路径的聚焦测试全部通过。
- 已验证 v4 数据库无损升级、导演降级提示上报与设置页性能改动。

## v2.8.3 — 小说拆解台、剧情导演与累计体验更新 (2026-09-12)

本版本合并 v2.8.2 之后的全部开发提交及本次已验收的工作区成果。

### ✨ 小说拆解台

- 支持 TXT、EPUB、拆解资料与小说世界资料包导入，识别编码和章节，预览原文并保留章节、偏移及来源版本。
- 按分析段提取证据、归并人物/势力/地点/物品档案并编译剧情；支持暂停、检查点恢复、有限错误修复和可选 Embedding，关闭向量检索仍可拆解。
- 世界资料转为世界书；剧情事件经独立原文引用校验后整理为导演主线，分析段不直接等同于游戏阶段。
- 接入主线整理、人工编辑、固定版本保存和小说世界创建；修复小说创建要求提示及主线来源引用问题。

### ✨ 剧情导演

- 原创原稿与小说共用主线定义和版本存储；支持人工修订历史、选用版本、自创/原作角色及阶段起点，仅在开局绑定固定版本。
- 正文前生成指导，正文后核对落实回执；按事实处理条件未知、拒绝、前提失效和人物死亡，已发生历史不被重新执行。
- 统一安排主线与人物幕后行动。主线耗尽后停止生成固定来源指令，游戏仍可继续发展。
- 变量系统保存人物真实状态，普通人物/任务面板使用玩家已知资料；重逢与披露只同步有依据的字段，幕后查看不更新角色认知。
- 幕后提案经变量和记忆管线受理；变量已提交后的重试保留原始事件内容，只补未完成部分，避免重复结算。
- 导演控制台提供总览、计划图谱、当前指令、落实回执、幕后事件、主线版本、依据诊断、机械日志与设置。
- 导演状态、玩家观察、记忆检查点与固定剧情依赖纳入存档、回滚和导入导出；修复首次保存遗漏及手动审查后快照未刷新的问题。

### 🔧 v2.8.2 之后的累计改进

- **世界创作**：支持从六个内置世界派生草稿并继续编辑；修复仅填写地理位置、时间或氛围时的保存遗漏，以及主线取消选用后的人工修订恢复。
- **NPC 与生图**：完善人物资料补全、属性归一化、头像和正文生图操作；支持确认后删除 NPC，并同步玩家已知资料。
- **物品栏**：增加玩家物品丢弃与即时刷新、NPC 物品栏展示，修复玩家面板没有传递变量管理器导致丢弃按钮不显示。
- **世界书**：统一世界书导入解析与合并，支持 SillyTavern、应用世界书和角色卡内嵌格式；空关键词且非常驻的条目不再意外全量注入。
- **玩法与管线**：完善玩法/战斗数值和事务边界、变量请求超时及状态快照；正文首次生成、空响应重试与格式补尾统一非流式请求。
- **界面与预设**：整理文风预设，优化移动端设置导航、人物档案、生命/资源状态条、世界书详情、变量快照和导演控制台布局。
- **速率限制**：小说拆解接入全局限流桶，按 provider 与接口地址共用设置中的「API 限流间隔」，不再只等触发 429 后才退避；等待期间显示剩余等待且可随时取消，收到 429 后按服务端要求自动延长间隔。

### 🧱 架构与发布

- 旧世界演化独立生成、暗线/事件推进和摘要注入退出；主线与人物计划归导演，机械周期结算归玩法系统。旧已确认历史仅保留读取与迁移用途。
- 内部数据保持单一规范结构，淘汰多代运行时兼容链；IndexedDB 当前为 v10，仅保留 v9 → v10 内部升级，外部导入协议单独校验。
- 静态资源统一由 `public/` 提供，构建复制到 Web 产物；清理旧入口、重复资源位置和本地报告产物。
- 首页、更新弹窗及事件兼容检查统一读取应用版本；更新弹窗按版本重新提示，README、功能总图和发布日志同步至 2.8.3。
- 发布流程校验版本与标签、执行类型检查/测试/Web 构建，并打包 Web 产物；当前源码不含 `src-tauri` 原生工程，原生构建任务仅在工程存在时运行。

### ✅ 验收记录

- 2.8.3 发布收尾：本地类型检查、856 项测试和 Web 生产构建各执行一次并全部通过，版本与发布日志校验通过。
- 最新功能快照报告：类型检查通过，856 项全量测试及 16 项额外聚焦回归通过；此前本地生产构建通过。
- 用户确认小说主线世界开局、Director 绑定和首轮回复正常。
- 用户确认导出 → 换设备/Profile → 导入 → 进入游戏 → 续玩 → 新回复落盘后，小说主线版本、阶段和绑定均保留，未退化为自由模式。
- 已覆盖的浏览器链路包含正常主线推进与耗尽、幕后变化及披露、变量与记忆、刷新、回滚、导入导出和主要面板。未覆盖的完整异常组合、战斗流程及云存档等仍待后续版本继续验证。

## v2.8.2 — 本地资产结构化编辑器与管线监控修复 (2026-08-29)

### ✨ 新增

- **本地资产结构化编辑器**：重做本地资产的结构化编辑界面
- **创意工坊简化**：简化创意工坊交互，补全本地资产编辑流程

### 🐛 修复

- **创意工坊分类与移动端**：修复创意工坊分类问题，优化移动端适配
- **管线监控报错可读性**：阶段报错（如变量提取失败）改为可「复制」、可「展开查看完整详情」的可滚动块，移动端无需 F12 即可查看完整报错，不再单行截断

## v2.8.1 — 世界降临仪式与角色创建 (2026-08-26)

### ✨ 新增

- **降临难度与点数**：简单、普通、困难、炼狱四档在创建第一步一次选定，同时决定战斗风险与统一降临点数池
- **职业牌阵**：职业会按当前典藏配置的数量以可翻面的塔罗牌阵展开，可在确认前查看道路定位、徽记与成长阶位
- **属性分配与命运抽卡**：属性、天赋直选和最多五次命运抽卡共享同一点数池；神技只可抽取，抽中后不重复计费
- **行囊纯化**：第三步集中处理初始段位、物品与同行者；无职业世界的属性加点也在此完成

### 🐛 修复

- **创建仪式视觉**：修正属性轨道端帽与填充对齐，以及职业牌阵的扇形布局

## v2.8.0 — 职业、技能与图形化战斗 (2026-08-25)

### ✨ 新增

- **两套职业典藏**：内置经典幻想与东方幻想基准包；每个职业拥有四阶、机械有效的能力节点、互斥专精与终极能力，典藏支持复制、编辑、导入导出和 AI 修订
- **统一能力协议**：先天天赋、职业能力、自由技能、动态技能、宠物/召唤能力和战斗道具共用 `AbilityDefinition`；AI 只提出语义方案，消耗、冷却、倍率与状态由本地配平器生成并等待玩家确认
- **独立图形化战场**：1v1 至 4v4 卡片战场，包含编队、目标选择、行动顺序、生命/资源/状态、伤害治疗反馈、退场动画、移动端布局和减少动态效果
- **完整战斗机械层**：每单位每轮一次行动，支持攻击、技能、道具、防御与逃跑；命中、伤害、治疗、暴击、状态、冷却和资源全部使用随机种子在本地确定性结算
- **本地策略 AI**：敌人和自动友方可使用进攻、均衡、防守、支援四种倾向，不在每个回合调用主模型
- **动态遭遇**：变量管线、类型化事件卡与规则工作流均可发起结构化遭遇；AI 只选择敌友身份、语境与威胁档位，不能直接指定任意战斗数值
- **战斗风险**：简单、普通、困难、炼狱四档独立于世界展示难度；简单战败恢复战前状态，炼狱玩家死亡后存档封存但保留浏览和导出能力

### 🔧 改进

- **事件选择闭环**：选项点击后立即应用效果、写入存档并自动保存，只在下一次成功正文中消费一次；空回或断网会保留待处理选择
- **完整战前检查点**：检查点覆盖变量、消息、记忆、世界推演与模块状态；普通和困难可恢复重打，炼狱禁止战斗重打与历史回滚
- **可靠战斗承接**：机械结算先落盘，再由主模型生成一次承接正文；空回、过滤和断网不会重复扣除道具、结算死亡或发放奖励
- **长期存档兼容**：旧职业包、天赋、技能、战斗历史与存档只增不删迁移，旧存档默认普通风险，未启用职业/战斗模块的世界保持原样
- **属性跨世界配平**：0–20 与 0–100 等不同世界属性范围统一归一化后参与战斗，群体技能、被动、熟练度和职业等级使用同一套受控倍率

### 🐛 修复

- **事件时序与串档**：删除"等待下一次世界 tick 才记录选择"的依赖，失败选择不丢失且不会跨存档注入
- **重复机械结算**：所有行动、消耗、奖励和终局事务使用幂等 ID，重复点击、刷新或恢复页面不会重复生效
- **战斗中后台漂移**：战斗与待承接阶段暂停普通消息、世界 tick 和记忆写入，结束后再统一进入叙事管线

## v2.7.6 — 世界时间与动态交互维护补丁 (2026-08-22)

### 🐛 修复

- **世界时间同步**：低算力模型漏写时间变量时，从正文中的次日、时段、明确日期和经过时长保守推断时间推进
- **手动时间修正**：变量面板修改纪年与时间时同步结构化时钟，保存后不会被旧值覆盖
- **正文解析兼容**：兼容服务返回多个正文标签、前置空标签等响应，不再误删后续有效正文
- **骰子卡片框体**：让正文内骰子检定卡使用面板框体与统一内边距，避免填充和边框错位

### ✨ 改进

- **世界动态边界**：明确动态摘要是后台推演，正文和当前变量始终优先，避免把"可介入"误解为面板内直接改写世界
- **动态建议交互**：事件建议与角色联系可填入主输入框，由下一轮正文正式承接
- **中文界面统一**：世界动态效果记录中的模型、角色、模块、训练和探索等系统标识改为中文

## v2.7.5 — 内置事件工作流重建 (2026-08-15)

### ✨ 重建

- **六世界触发工作流**：为六个内置世界恢复独立、可视、可开关的事件触发工作流，40 个事件按世界时间稳定轮转，不再与事件内容包混为一层
- **正文叙事承接**：玩家在事件卡中的选择会携带事件与选项语境进入下一轮 AI 正文

### 🐛 修复

- **事件选择失效**：自动修复 17 个旧事件中的非标准选项 JSON
- **属性效果失效**：将旧版生命/体力属性名迁移为规范键，避免选择效果被静默跳过
- **工作流误触发**：修复流程条件为 false 时下游动作仍会执行的问题

## v2.7.4 — Agent 共创重构 + DeepSeek 预设 + 游戏运行追踪与稳定性增强 (2026-08-13)

### ✨ 新增

- **Agent 共创重构**：自定义模块代理系统全面重构，增强 AI 自主装配世界模块能力
- **DeepSeek 专用预设**：新增 DeepSeek API 专用预设，优化流式响应处理
- **游戏运行追踪**：新增游戏过程运行状态追踪，增强稳定性

### 🐛 修复

- **纪事系统 null 崩溃**：修复部分情况下纪事系统空值导致的崩溃问题
- **音效重构**：优化音频播放逻辑，提升稳定性 **晨光秘境旅庭美工重构**：全面升级 Dawn V4 视觉系统，统一全界面设计语言
- **对话预测卡片**：新增 `ui:DL` 格式解析，AI 对话预测输出自动渲染为带头像的对话卡片
- **大厅背景音乐**：世界大厅添加《Scarborough Fair》背景音乐（仅大厅播放，首页/过场/游戏过程不播放），右下角静音按钮
- **玩家头像**：角色信息面板顶部新增玩家头像展示（支持默认剪影 + 自定义头像）

### 🔧 改进

- **导航图标更新**：游戏主界面 11 个导航按钮图标全部更新为对应主题编号
- **对话卡片去框**：`InlineDialogueCard` 移除 DawnFrameV4 装饰外框，改为简洁头像+气泡布局

### 🐛 修复

- **移动端向导 Step 1 滚动**：修复移动端无法滚动查看完整表单的问题
- **移动端向导 Step 2 溢出**：修复天赋与行囊展开内容在窄屏幕上超出边界
- **选项卡片多选假象**：修复点击多个选项时全部显示选中状态的视觉 bug
- **删除未使用的根目录图片**：清理 `bg-main.png`、`bg-main-phone.png`、`title-main.png`

## v2.7.2 — 自定义模块代理 — AI 自主装配世界模块 (2026-08-06)

### ✨ 新增

- **自定义模块代理**：AI 可自主装配和配置世界模块，支持运行时动态创建与调整模块参数
- **模块代理引擎桥接**：`runCustomModulesForWorldAndCommit` 集成到回合结束和 tick 循环中，模块代理输出自动写回游戏状态

### 🔧 改进

- **移动端断点调整**：游戏界面移动端触发从 900px 降至 640px，平板设备使用优化版桌面布局
- **ChatPanel 样式重构**：从内联样式迁移到 CSS 类，统一 Dawn V4 设计规范
- **MessageBubble 样式重构**：气泡样式从内联迁移到 CSS 变量驱动
- **启动屏幕**：新增入口过渡动画 (`EntryTransition`) 与大厅视图分离
- **旅途卷宗面板**：统一面板标题/图标元数据 (`journeyDossierMeta`)，规范化桌面与移动端面板渲染

### ✨ 新增

- **事件包格式统一**：废弃旧版 `EventPack` 格式，全面迁移到 `CanonicalEventPack` 规范，统一事件、规则、工作流的数据结构
- **卡片工作流重构**：`CardWorkflowEngine` 重写，节点注册表新增 25 种类型化节点，支持 DAG 拓扑执行、Socket 颜色区分、自动布局
- **世界书深度融合**：内置 6 个世界恢复完整 `worldBookEntries`，新增 `NPCWorldbookGenerator` 自动从 NPC 数据生成世界书条目，支持去重、递归扫描
- **测试补齐**：新增 12 个测试文件（EventIdSelect、StepWorldBrowser、pipelineExecutor、cardWorldBindings、enabledEventPack、eventPackFormat、eventPackLegacyGuard、workflowConverters、simulation engine 等）
- **新增 6 个世界图标**：TreePalm、Plane、Sailboat、Tent、TentTree、Trees，丰富世界选择视觉表现

### 🔧 改进

- **事件导入/导出**：统一错误处理，新增 `EventIdSelect`、`EventPackBadge`、`EventPackPreview` 组件
- **工作流转换器**：`workflowConverters.ts` 支持工作流 ↔ 规则双向转换，新增端口对齐验证
- **遗留数据迁移**：`eventPackLegacyGuard.ts` 强化旧格式拦截，`eventPackFormat.ts` 规范化导入流程
- **卡片世界绑定**：`cardWorldBindings.ts` 新增世界级卡片工作流绑定与激活逻辑

## v2.7.0 — AI 合集生成器 (2026-07-24)

### ✨ 新增

- **AI 合集生成器**：事件中心新增「AI 生成」按钮，选中世界后一键生成包含事件+规则的完整内容合集
- **世界书深度融合**：AI 自动读取世界的属性、资源、NPC、势力等配置，生成的 widgetValues 精确匹配世界实际值
- **规则-事件自联动**：规则可通过 `actions.add_event` 弹出本合集中的事件卡，实现后台规则触发 → 弹出事件的联动效果
- **prompt 系统**：新增 `event-generator.ts`，构建包含节点白名单、连线规则、自联动示例的完整 prompt

## v2.6.9 — 内置世界卡片事件包重建 (2026-07-24)

### ✨ 新增

- **内置世界事件包重建**：40 个新 CardWorkflowDefinition 格式事件
- **清理旧世界 JSON**：移除旧格式事件数据
- **worldWorkflows.ts 清空**：工作流数据迁移到新格式

## v2.6.8 — 动态任务系统 + 纪事系统 (2026-07-24)

### ✨ 新增

- **动态任务系统**：从记事本"待办事项"升级为独立任务系统，支持主线/支线/日常/隐藏/成就 5 种任务类型
- **深度游戏联动**：任务支持 7 种条件（物品/属性/资源/技能/NPC好感度/骰子检定/货币）+ 丰富奖励（经验/金币/物品/技能/天赋/属性/好感度/资源/解锁任务/解锁段位）
- **多阶段任务链**：复杂任务可拆分为多个阶段，每阶段独立条件和奖励
- **任务面板 UI**：独立任务面板（🎯），支持类型过滤、优先级排序、进度条、阶段指示器、需求条件检查、奖励展示
- **纪事系统**：原"记事本"改造为统一情报板，支持 6 种类型标签（风险/机遇/线索/关系/地点/物品）
- **纪事过滤标签**：按类型过滤纪事，各类型独立计数
- **旧存档兼容**：自动迁移旧"记事本"数据到新纪事系统 + 任务系统

### 🔧 改进

- **变量提取 Prompt**：新增完整任务系统规则，AI 自动创建/推进/完成任务
- **工作流节点**：`add_notebook` 节点适配新纪事系统，支持 6 种类型选择
- **NPC 快照**：npcHelpers 改为读取纪事系统
- **CSS Token 统一**：278 处硬编码 px 值替换为 CSS 变量（19 个文件），覆盖 padding/margin/gap/border-radius/width/height
- **本地资产 Tab**：用户中心新增第 4 个 Tab「本地资产」，管理本地世界包/人物预设/NPC 模板/人生经历，支持一键上传到创意工坊

## v2.6.7 — 工作流系统 (2026-07-23)

### ✨ 新增

- **工作流系统**：25 种类型化节点 + DAG 执行引擎
- **世界绑定**：工作流与世界绑定
- **引擎集成**：工作流引擎集成到游戏引擎
- **卡片触发修复**：修复事件卡片触发问题

## v2.6.6 — 事件包ID修正 (2026-07-22)

### 🐛 修复

- **事件包ID冒号支持**：事件包 ID 支持冒号字符
- **CardEditor世界绑定**：卡片编辑器支持世界绑定
- **规则路径修正**：修复规则路径问题
- **WhenPathSelect中文路径**：When 路径选择器支持中文路径

## v2.6.5 — 云存档优化 (2026-07-21)

### 🔧 改进

- **云存档 gzip 压缩**：云存档传输启用 gzip 压缩
- **存档裁剪**：存档数据裁剪优化
- **SW 清理**：Service Worker 缓存清理优化

### 🐛 修复

- **创意工坊上传修正**：修复创意工坊上传问题

## v2.6.4 — 云存档 + 创意工坊 + 邮箱登录 (2026-07-20)

### ✨ 新增

- **云存档系统**：基于 Cloudflare Workers + D1 的云存档同步
- **创意工坊**：支持作品上传和分享
- **邮箱验证码登录**：基于 Brevo 邮件服务的邮箱验证码登录

## v2.6.3 — 事件系统命名重构 + 记忆系统接资源 (2026-07-15)

### ✨ 新增

- **事件系统命名重构**：统一事件系统相关命名规范
- **记忆系统接资源**：记忆系统接入资源管理
- **世界演化改用记忆系统输入**：世界演化引擎改为从记忆系统获取输入
- **内置世界清空 eventPacks**：内置世界配置清空 eventPacks 字段

## v2.6.2 — 规则引擎重构 + When条件编辑器 (2026-07-14)

### ✨ 新增

- **规则引擎重构**：规则引擎全面重构，提升可扩展性
- **世界绑定**：规则与世界绑定
- **When 条件编辑器**：可视化编辑规则触发条件
- **移动端适配**：规则编辑器移动端适配

## v2.6.1 — 事件中心二级开关 + 预览折叠列表 (2026-07-13)

### ✨ 新增

- **事件中心二级开关**：事件中心新增二级开关控制
- **预览折叠列表**：预览面板支持折叠列表展示
- **内置包自动安装**：内置事件包自动安装
- **分区标签修正**：修正事件分区标签显示
- **主页 Beta 测试标签**：主页新增 Beta 测试标签

## v2.6.0 — 事件系统重构四级模型 (2026-07-13)

### ✨ 新增

- **事件系统重构**：库→包→事件→卡四级模型
- **规则引擎**：新增规则引擎系统
- **Rust 后端**：事件系统 Rust 后端支持
- **Web 端 IndexedDB**：Web 端事件数据 IndexedDB 持久化

## v2.5.0 — 发布就绪改造 + 功能增强 (2026-07-12)

### 🔒 安全 / 稳定（G0）

- **L-01** API Key 加密存储（Web Crypto AES-GCM non-extractable + IndexedDB）
- **L-02** iframe DOMPurify 净化 + sandbox=allow-scripts（移除 allow-same-origin 防沙箱逃逸）
- **L-03** Tauri CSP 最小白名单（非 null）
- **L-04** 流式超时 120s AbortController 真正 abort（消除永久卡死）
- **L-05** 依赖钉定精确版本 + 移除冗余 @types/uuid/@types/dompurify
- **L-06** build.ts 全量输出多 chunk
- **L-07** Tauri devUrl→3456 + http:default 能力

### 📄 发布就绪（G1）

- **L-08** 新增 LICENSE（MIT）、CONTRIBUTING、PRIVACY.md
- **L-09** 新增用户教程（配 Key→建世界→发消息→回滚变量全流程）
- **L-10** 新增独立落地页（docs/site/index.html）
- **L-11** 新增内容政策 + 社区准则
- **L-12** 新增治理文档（永久开源免费）
- **L-13** 语义色全量 token 化（var() 替代硬编码色值）
- **L-14** 桌面 DrawerPanel a11y（role/aria-modal/Esc/焦点陷阱）
- **L-15** 桌面图标导航补 aria-label
- **L-16** 核心纯函数单测（variableManager/responseExtractor/rateLimiter/db迁移）
- **L-17** 429 尊重 Retry-After + 分桶限流
- **L-18** 补最小 sw.js（PWA 可安装）
- **L-19** 原型污染防护（isSafePath/containsDangerousKey）
- **L-20** @types/uuid 版本错配清理 + typecheck 脚本

### ⚡ 功能增强

- **ComfyUI 工作流管线升级**：格式自动检测 + 拓扑注入 + 资源校验 + 代理穿透
- **世界演化引擎**：注入玩家对话上下文，解决前后台叙事不同步
- **反八股模块化改造**：禁用词扩充 + 6 个可开关子模块 + 正则后处理安全网
- **生存系统增强**：配方存档持久化 + 运行时资源元数据显示 + 产出校验

## v2.2.0 — 全量代码审查修复 + 延迟优化 + 记忆空间感知 (2026-07-06)

### 🐛 Bug 修复

- **模拟规则匹配**：修复 `matchEventEffect` 无 keywords 时永远返回 false，导致依赖 tag/eventType 的规则全部失效
- **JSON 修复正则**：修复 `repairJSON` 中 `,s*` typo（应为 `,\s*`），尾逗号修复现在能正确处理空格/换行
- **维度值静默丢失**：修复 dim 键名不匹配（`dim1Value` vs `dim1`），维度值不再回退为默认 50
- **setvar 宏截断**：修复 `{{setvar::key::value}}` 在值包含 `}` 时截断的问题
- **brewing 事件零生命周期**：brewing 事件现在至少存活 1 tick 才升级为 active
- **entity stableFacts 覆盖**：pipeline 版本的实体档案写入改为累积 stableFacts，与 hook 版本一致，不再丢失历史事实
- **retrySingleStage 竞态**：修复单步重试未设置 `generatingRef`，导致可并发发送请求

### ⚡ 性能优化

- **世界演化非阻塞**：`simEngine.tick()` 改为 fire-and-forget 后台执行，正文生成不再等待世界演化 API 调用（节省 2-15 秒）
- **速率限制器默认值**：从 10 秒降到 3 秒，减少管线阶段间等待（可通过 `detectOptimalRateLimit` 自动调整）
- **变量提取延迟**：移除硬编码 3 秒 sleep，仅保留可配置延迟（默认 1 秒）
- **MessageBubble memo**：添加 `React.memo` + `useCallback` 稳定回调，避免聊天列表不必要的重渲染
- **限流日志降级**：`console.log` 改为 `console.debug`，减少控制台噪音

### 🛡️ 健壮性

- **useMemorySystem.callAI 超时**：添加 120 秒超时保护，防止 API 挂起导致无限等待
- **Chat Error Boundary**：每条消息独立 Error Boundary，单条消息渲染失败不影响整体聊天

### 🧠 记忆空间感知（解决记忆混乱）

- **类型扩展**：`relationNetwork`/`relationEdge` 新增 `locationScope` 字段，`entityCards` 新增 `locationFacts` 带地点的事实数组
- **写入提示词改造**：CoT 增加空间检查步骤，事件卡强制填写 `locationRefs`，关系要求填写 `locationScope`，实体事实要求包含地点信息
- **sceneAnchor 替换式更新**：不再用 `??` 回退旧值，AI 返回空即清空，防止旧地点的 goal/risk 泄漏到新场景
- **地点变化自动降级**：切换地点时，旧地点的空间关系从 `active` 自动降级为 `changed`
- **编译输出空间排序**：关系选择按当前地点排序（当前地点优先），格式化输出包含 `[地点]` 标注
- **模拟快照指数膨胀修复**：`_slimForSnapshot` 未排除 `snapshots` 字段导致每个快照嵌套所有旧快照（119MB → ~1MB）

## v2.1.0 — Simulation模块配置系统 + 世界演化规则编辑器 (2026-07-05)

### ✨ 新增

- **Simulation模块配置系统**：创建世界时可选择"世界演化"模块，自动注入默认规则
- **游戏内规则编辑器**：在世界动态面板的"设置" tab 中配置演化规则
- **事件效果编辑**：支持配置触发条件（关键词/标签）和效果（联动生存/属性/经营模块）
- **周期事件编辑**：支持配置周期间隔、首次偏移、效果
- **安全护栏配置**：属性/资源最大变动、允许AI创建新资源
- **导入导出系统**：规则 JSON 可复制到剪贴板，方便分享和复用
- **5个世界预设规则**：边贸风云、日式校园、孤岛求生、余烬废土、江湖风云录
- **烟火人间**：新增都市日常题材世界（原霓虹迷都重构）

### 🔧 改进

- **模块选择器**：新增"世界演化"选项
- **SimSettings**：集成规则编辑器，可展开/折叠
- **世界定义**：6个内置世界全部配备世界演化规则

## v2.0.0 — Tauri 桌面应用 + 自动更新 (2026-07-05)

### ✨ 新增

- **Tauri 桌面应用**：支持打包为 Windows .exe/.msi 安装包
- **自动更新**：内置 Tauri 更新器，支持 GitHub Releases 自动更新
- **GitHub Actions**：自动化构建和发布工作流
- **防八股模块**：AntiFormula 预设，解决 AI 输出重复问题

### 🔧 改进

- **版本号统一**：所有版本号同步更新到 2.0.0
- **窗口优化**：默认大小 1280x800，最小 800x600
- **安装包优化**：支持 NSIS 和 MSI 两种安装格式

## v1.9.9 — 世界演化规则系统 + 变量提取联动重构 (2026-07-04)

### ✨ 新增

- **世界演化规则系统**：SimulationRules 类型定义、机械层结算器、统一应用器
- **世界状态泛化更新**：worldStateAxes 替代写死的社会环境/信息层级字段
- **AI 生成规则提示词**：buildSimulationRulesPrompt + validateSimulationRules
- **叙事层护栏**：validateNarrativeEffects 校验 AI 越界声明
- **可观测性**：EffectLogTab 面板展示变量变化日志
- **性能索引**：tag→rule/keyword→rule 索引优化匹配速度
- **快照系统**：世界演化快照 + 变量/记忆/世界演化三系统联动回滚

### 🔧 改进

- **变量提取联动**：世界演化输出 mechanicalEffects，变量提取统一处理
- **前端数据源修复**：SurvivalCard 从 GameState 读取运行时值
- **NPC 属性结构统一**：属性字段改为生存状态，与玩家一致
- **hasProgression 支持等级制**：levelData 判断生效
- **移除社会环境/信息层级**：精简 WorldState 结构

### 🐛 修复

- **NPC survivalStats 写入**：useStartScreen.ts 正确写入生存状态
- **npcHelpers.ts 字段引用**：ext.属性 → ext.生存状态
- **NPCDetail.tsx 字段引用**：ext.属性 → ext.生存状态

## v1.9.8 — 变量瘦身与 UI 去重 (2026-07-03)

### 🔧 改进

- **变量系统瘦身**：删除 `有效期`、`特殊属性`、`次级货币`、`阶层`、`所属组织`、`特殊身份`、`主流价值观`、`纪元名称`、`区域特征` 共 9 个冗余空字段，减少每轮 JSON 体积
- **UI 去重**：左档案栏移除「世界状态」和「当前目标」（右面板已有），右面板移除「待办事项」（笔记本已有）
- **SSE 流解析优化**：`response.body` 空值检查 + flush 代码去重
- **ComfyUI 轮询优化**：连续错误 15 次提前终止，不再等 5 分钟
- **sendMessage 依赖优化**：移除多余的 `isGenerating` 依赖

### 🐛 修复

- **变量提取错误不再被静默吞掉**：管线变量提取失败时正确抛出错误，防止保存未更新变量的快照
- **推演引擎状态突变修复**：`applyChronicleOps` 先复制再修改，避免直接 mutate 游戏状态
- **经营模块 Bug 修复**：变量泄漏到正文、经营模块不更新、结算逻辑缺陷、世界书标记错误
- **类型声明补全**：新增 `global.d.ts` 解决 CSS Module 和 highlight.js 类型错误
- **RightPanel 导入修复**：补上缺失的 `BusinessModuleSchema` 导入

## v1.8.0 — 世界创建多选支持 (2026-06-27)

### ✨ 新功能

- **GuidedChoiceOverlay 多选支持**：地理格局、势力结构、关键人物、文化风俗维度支持多选（最多 2-3 个选项）
- **自定义选项 E 卡片**：多选模式下支持添加自定义选项，并正确维护选择状态

### 🔧 改进

- **conflict 维度持久化**：核心冲突选择现在会作为结构化数据保存到 setting 条目的 meta 中
- **assembler 多选支持**：世界书条目组装器现在正确处理多选维度，为每个选择生成独立条目

### 🐛 修复

- **stale closure bug**：修复多选切换时快速点击导致选择丢失的问题
- **自定义选项选中状态**：修复多选模式下自定义 E 卡片不显示选中高亮的问题
- **handleSaveCustom 多选支持**：修复保存自定义选项时替换所有多选选择的问题
- **AbortController 泄漏**：修复 AI 请求控制器未正确中止的问题
- **dead import 清理**：移除未使用的 generateAllOptions 导入

## v1.7.1 — 死代码清理 + 结构优化 (2026-06-26)

### 🔧 改进

- **统一 `findWorldDef`**：删除 `useGameEngine.ts` 中的重复定义，统一使用 `worldLoader.ts` 导出
- **提取管线降级工厂函数**：消除 `useGameEngine.ts` 中 12 处重复的降级检测代码
- **修复硬编码 key**：`worldLoader.ts` 中 `localStorage.getItem('world_travel_guide_custom_worlds')` 改为 `STORAGE_KEYS.CUSTOM_WORLDS`

### 🗑️ 清理

- 删除死文件：`src/types/api.ts`、`src/types/engine.ts`、`src/utils/moduleToWorldBook.ts`
- 删除死函数：`assembleSystemPromptLegacy`、`updateLastCallTime`、`createRoleCognitionFirewallPresetEntry`、`useMediaQuery`
- 删除死常量：`DEFAULT_TEXT_COLORIZATION_RULE_IDS`

### 🐛 修复

- 修复 `MessageBubble` 中 `getActivePreset()` 作为 Zustand selector 每次返回新对象引用导致的无限重渲染

## v1.7.0 — 天赋体系模块 + 记忆管线并行化 + 内置世界重构 (2026-06-21)

### ✨ 新功能

- **天赋体系模块**：完整的天赋系统，支持天赋大类和具体天赋生成，品质分为普通/精良/稀有/史诗/传说五档
- **天赋觉醒卡片**：AI 在叙事中触发天赋觉醒时，渲染内联天赋卡片（类似骰子检定卡片）
- **模块依赖显示**：选择成长体系时，数值属性自动显示"自动启用"标签

### 🔧 改进

- **记忆管线并行化**：写入阶段（叙事记忆、摘要保存、向量提取）改为并行执行，提升约 60-70% 性能
- **冲突裁决并行化**：事件卡和实体卡的冲突裁决改为 Promise.all 并行处理
- **天赋世界书集成**：天赋模块勾选后，自动注入天赋规则到世界书（绿灯触发）
- **内置世界重构**：重新规划 6 个内置世界，覆盖所有模块组合
  - 日式校园（无模块）→ 入门纯文游
  - 欲望魔都（数值）→ 都市题材
  - 武侠世界（数值+成长）→ 武侠题材
  - 末日废土（数值+成长+天赋+骰子）→ 异能冒险
  - 荒岛求生（生存资源）→ 生存题材
  - 边境贸易（经营资产）→ 经营题材
- **世界卡片样式重构**：提升卡片质感，添加难度标签，优化视觉效果

### 📦 技术变更

- 新增 `TALENT_RULES_PROMPT` 常量，定义天赋觉醒规则和 `[TALENT_GAIN]` 标记格式
- 新增 `InlineTalentCard` 组件，用于渲染天赋觉醒内联卡片
- 更新 `buildPipeline.ts`，添加天赋体系生成逻辑
- 更新 `pipelineTypes.ts`，写入阶段改为并行执行
- 删除赛博朋克、绯晶之乡、云汉皇朝·深宫世界文件
- 新增荒岛求生、边境贸易世界文件

---

## v1.6.3 — 删除核心锚点 + 管线单步重试 (2026-06-20)

### ✨ 新功能
- 管线单步重试：监控弹窗中失败的 API 阶段显示小重试按钮，支持单独重试某个阶段而非整条重试

### 🔧 改进
- 删除核心锚点（与人物事迹功能重复），节省 AI Token 消耗
- 管线重试按钮仅在需要调用 API 的阶段显示，本地阶段（检索收尾、上下文编译）不显示

## v1.6.1 — 生存资源制作系统 + SW 缓存修复 (2026-06-19)

### ✨ 新功能

- **生存资源制作系统**：支持配方生成、手动制作、资源消耗与产出，完整闭环
- **配方 AI 生成**：输入需求自动生成 JSON 配方，带错误容错和 ID 唯一保证
- **配方管理 UI**：制作面板支持查看配方列表、一键制作、删除配方

### 🔧 改进

- **生存资源配置提取**：补全 `amount` 字段提取，修复变量提取漏字段问题
- **apiConfig TDZ 修复**：将 `useConfigStore` 调用移到回调定义之前，消除变量提升错误
- **Vite 残留清理**：统一为 Bun 构建，移除所有 Vite 相关引用
- **Service Worker 缓存策略**：开发环境 `/app.js` 改为网络优先，避免缓存旧代码

---

## v1.6.0 — NPC 模块同步 + NSFW 重写 + PWA 支持 (2026-06-19)

### ✨ 新功能

- **NPC 模块同步**：NPC 继承世界的数值属性和成长体系，AI 补全和变量提取均支持 NPC 属性/段位更新
- **NSFW 预设重写**：无限制模式升级为六阶段流程（铺垫→探索→升温→攀升→释放→余韵），新增词汇规范和负面清单
- **PWA 支持**：Service Worker 缓存优先策略 + Web App Manifest，支持安装到桌面
- **变量结构扩展**：新增 NPC 段位（`npc_tier`）和经验值（`npc_xp`）变量定义

### 🔧 改进

- **加强预设破限**：默认模式和沉浸模式的创作指令增强，扩展角色年龄设定规则
- **世界编辑器防误触**：点击遮罩层不再关闭面板，防止意外丢失编辑内容
- **移动端头部优化**：显示世界名称而非世界 ID
- **世界卡片布局**：桌面端改为单列显示，间距加大
- **存档导入修复**：支持 `{save: {...}}` 嵌套格式的存档文件导入
- **存档导出文件名**：改为 `world-wanderer-save-{timestamp}.json`
- **ProfilePanel 去重**：移除与 RightPanel 重复的生存状态显示

### 🏗 技术架构

- React 18 + TypeScript + Zustand
- Bun 开发环境 + 构建打包
- IndexedDB 持久化存储
- CSS Layers 设计系统 (base/layout/state/theme/print/a11y)
- PWA: Service Worker + Web App Manifest

---

## v1.0.0 — 正式发布 (2026-06-13)

首个正式版本。

### ✨ 核心功能

- **AI 对话引擎**：流式响应，支持 OpenAI / DeepSeek / Google AI / 自定义端点
- **多阶段管线**：正文生成 → 记忆写入 → 摘要保存 → 向量提取 → 检索规划 → 上下文编译 → 变量提取
- **记忆系统**：编译式叙事记忆引擎（场景锚点、叙事线程、状态槽、关系边、事件卡、实体档案）
- **图谱可视化**：13 种 Mermaid 图谱视图
- **变量系统**：快照 / 回滚 / RFC 6902 Patch / NPC 感知合并
- **存档系统**：多存档 + 自动存档 + 导入导出 + F5 恢复
- **预设系统**：15 个结构化提示词模块 + 宏引擎 + 正则脚本
- **游戏向导**：5 步式角色创建（世界选择 → 角色创建 → 人物经历）
- **内置世界**：7 个世界观 + 世界编辑器

### 🏗 技术架构

- React 18 + TypeScript + Zustand
- Bun 开发环境 + 构建打包
- IndexedDB 持久化存储
