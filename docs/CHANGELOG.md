...
- **AI 合集生成器**：事件中心新增「AI 生成」按钮，选中世界后一键生成包含事件+规则的完整内容合集
- **世界书深度融合**：AI 自动读取世界的属性、资源、NPC、势力等配置，生成的 widgetValues 精确匹配世界实际值
- **规则-事件自联动**：规则可通过 `actions.add_event` 弹出本合集中的事件卡，实现后台规则触发 → 弹出事件的联动效果
- **prompt 系统**：新增 `event-generator.ts`，构建包含节点白名单、连线规则、自联动示例的完整 prompt

## v2.6.9 — 内置世界卡片事件包重建 (2026-07-24)

### ✨ 新增

- **内置世界事件包重建**：40 个新 CardWorkflowDefinition 格式事件
- **清理旧世界 JSON**：移除旧格式事件数据
- **worldWorkflows.ts 清空**：工作流数据迁移到新格式

## v2.6.8 — 动态任务系统 + 纪事系统 (2026-07-24)

### ✨ 新增

- **动态任务系统**：从记事本"待办事项"升级为独立任务系统，支持主线/支线/日常/隐藏/成就 5 种任务类型
- **深度游戏联动**：任务支持 7 种条件（物品/属性/资源/技能/NPC好感度/骰子检定/货币）+ 丰富奖励（经验/金币/物品/技能/天赋/属性/好感度/资源/解锁任务/解锁段位）
- **多阶段任务链**：复杂任务可拆分为多个阶段，每阶段独立条件和奖励
- **任务面板 UI**：独立任务面板（🎯），支持类型过滤、优先级排序、进度条、阶段指示器、需求条件检查、奖励展示
- **纪事系统**：原"记事本"改造为统一情报板，支持 6 种类型标签（风险/机遇/线索/关系/地点/物品）
- **纪事过滤标签**：按类型过滤纪事，各类型独立计数
- **旧存档兼容**：自动迁移旧"记事本"数据到新纪事系统 + 任务系统

### 🔧 改进

- **变量提取 Prompt**：新增完整任务系统规则，AI 自动创建/推进/完成任务
- **工作流节点**：`add_notebook` 节点适配新纪事系统，支持 6 种类型选择
- **NPC 快照**：npcHelpers 改为读取纪事系统
- **CSS Token 统一**：278 处硬编码 px 值替换为 CSS 变量（19 个文件），覆盖 padding/margin/gap/border-radius/width/height
- **本地资产 Tab**：用户中心新增第 4 个 Tab「本地资产」，管理本地世界包/人物预设/NPC 模板/人生经历，支持一键上传到创意工坊

## v2.6.7 — 工作流系统 (2026-07-23)

### ✨ 新增

- **工作流系统**：25 种类型化节点 + DAG 执行引擎
- **世界绑定**：工作流与世界绑定
- **引擎集成**：工作流引擎集成到游戏引擎
- **卡片触发修复**：修复事件卡片触发问题

## v2.6.6 — 事件包ID修正 (2026-07-22)

### 🐛 修复

- **事件包ID冒号支持**：事件包 ID 支持冒号字符
- **CardEditor世界绑定**：卡片编辑器支持世界绑定
- **规则路径修正**：修复规则路径问题
- **WhenPathSelect中文路径**：When 路径选择器支持中文路径

## v2.6.5 — 云存档优化 (2026-07-21)

### 🔧 改进

- **云存档 gzip 压缩**：云存档传输启用 gzip 压缩
- **存档裁剪**：存档数据裁剪优化
- **SW 清理**：Service Worker 缓存清理优化

### 🐛 修复

- **创意工坊上传修正**：修复创意工坊上传问题

## v2.6.4 — 云存档 + 创意工坊 + 邮箱登录 (2026-07-20)

### ✨ 新增

- **云存档系统**：基于 Cloudflare Workers + D1 的云存档同步
- **创意工坊**：支持作品上传和分享
- **邮箱验证码登录**：基于 Brevo 邮件服务的邮箱验证码登录

## v2.6.3 — 事件系统命名重构 + 记忆系统接资源 (2026-07-15)

### ✨ 新增

- **事件系统命名重构**：统一事件系统相关命名规范
- **记忆系统接资源**：记忆系统接入资源管理
- **世界演化改用记忆系统输入**：世界演化引擎改为从记忆系统获取输入
- **内置世界清空 eventPacks**：内置世界配置清空 eventPacks 字段

## v2.6.2 — 规则引擎重构 + When条件编辑器 (2026-07-14)

### ✨ 新增

- **规则引擎重构**：规则引擎全面重构，提升可扩展性
- **世界绑定**：规则与世界绑定
- **When 条件编辑器**：可视化编辑规则触发条件
- **移动端适配**：规则编辑器移动端适配
