...
*文档生成时间: 2026-09-04 → 2026-09-08 重组 → 2026-09-08 逐字节核对修订*
  
*审查方法: 子系统级逐文件目录盘点 + 与原文档交叉核对 + 逐字节代码核对(累计修复 60+ 处(本轮新增:3 高/5 中/8 低):1 个类名/13 个事件常量值/2 个接口签名/4 个安全常量/2 个不存在的接口方法/6 个 §2.2 错误方法名/3 个 novel 不存在文件/13 个行数偏差/2 个 §6.6 描述/1 个 DB_VERSION/1 个 character_card 规格/1 个 §22.1 原则冲突/1 个 styles 数量)*
  
*总行数覆盖: ~17000行核心代码 + 27个 src 子目录(含 **tests**/dev) + docs/migrations/scripts/src-tauri 根目录子系统*
  
*重组要点: 新增 §0 总览 / §5 gameplay / §6 simulation / §7 custom-modules / §10 Context / §11 Hooks / §12 data / §13 novel / §14 time / §15 worldgen / §16 主题 / §17 安全基础设施 / §18 utils / §19 根目录 / §2十 docs;原 §5~§9 重新编号 §8 / §9 / §2十一 / §2十二;原 §7 模块系统并入 §4.5;原 §10~§15 散节内容并入 §8 / §9 / §2十三*
