...
interface RegistryPresentationProps { goBack: () => void; isAuthenticated: boolean; isLoading: boolean }
...
function ConnectedUserCenter() {
  ...
  return <RegistryPresentation goBack={goBack} isAuthenticated={isAuthenticated} isLoading={isLoading} />;
}

function RegistryPresentation({ goBack, isAuthenticated, isLoading }: RegistryPresentationProps) {
...
function SignedInRegistry({ tab, setTab }: { tab: UserTab; setTab: (tab: UserTab) => void }) {
