...
export function NovelImportWorkbench({ onClose, onCreateWorld, worlds = [], onUpdateWorld }: NovelImportWorkbenchProps) {
  ...
  const startAnalysis = async (regenerate = false) => {
    ...
    try {
      ...
      const run = runNovelAnalysis({ datasetId: dataset.id, config: { ...config.api, rateLimitMs: config.analysisRateLimitMs }, startSegmentIndex, embedding: makeEmbedding(), forceOverview: regenerate, signal: controller.signal, onCheckpoint: setDataset, onProgress: value => { setProgress(value); setMessage(value.message); if (value.embeddingStatus) setEmbeddingState(value.embeddingStatus); } });
