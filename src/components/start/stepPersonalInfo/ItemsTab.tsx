        ...
        <div key={name} className="world-dynamic-item" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', flex: 1 }}>
            ...
            <div style={{ display: 'flex', gap: '6px' }}>
              <input type="number" value={item?.数量 ?? 1} onChange={e => updateItemField(name, '数量', e.target.value)} min={1} style={{ fontSize: 'var(--font-size-base)', padding: '6px 8px', width: '50px' }} placeholder="数量" />
              <select value={item?.品质 ?? '普通'} onChange={e => updateItemField(name, '品质', e.target.value)} style={{ fontSize: 'var(--font-size-base)', padding: '6px 8px', border: '1px solid var(--border)', borderRadius: '6px', background: 'var(--bg-secondary)', width: '70px' }}>
