      ...
      {Object.entries(personalInfo.initialSkills).filter(([_, s]) => s != null).map(([name, skill]) => (
        <div key={name} className="world-dynamic-item" style={{ flexDirection: 'column', alignItems: 'stretch' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-1)', flex: 1, minWidth: 0 }}>
            <input type="text" defaultValue={name} onBlur={e => renameSkill(name, e.target.value)} placeholder="技能名称..." style={{ fontSize: 'var(--font-size-base)', padding: '6px 8px', maxWidth: '100%', boxSizing: 'border-box' }} />
            <select value={skill?.品质 ?? '普通'} onChange={e => updateSkillField(name, '品质', e.target.value)} style={{ fontSize: 'var(--font-size-base)', padding: '6px 8px', border: '1px solid var(--border)', borderRadius: '6px', background: 'var(--bg-secondary)', maxWidth: '100%', boxSizing: 'border-box' }}>
              {QUALITY_OPTIONS.map(q => <option key={q} value={q}>{q}</option>)}
            </select>
            <input type="text" value={skill?.描述 ?? ''} onChange={e => updateSkillField(name, '描述', e.target.value)} placeholder="技能描述..." style={{ fontSize: 'var(--font-size-base)', padding: '6px 8px', maxWidth: '100%', boxSizing: 'border-box' }} />
          </div>
