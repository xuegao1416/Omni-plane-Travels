...
const optionalParameters = ['parallel_tool_calls', 'reasoning_effort', 'top_k'] as const;

function explicitlyRejectsParameter(message: string, parameter: string): boolean {
  const name = `[\"'\x60]?\\b${parameter}\\b[\"'\x60]?`;
  return new RegExp(`(?:unknown|unsupported|unrecognized|unrecognised|unexpected)\\s+(?:(?:request|body)\\s+)?(?:parameter|argument|field|name)s?(?:\\s+supplied)?\\s*:?\\s*${name}(?![.\\[])|${name}\\s*(?:is\\s+|are\\s+)?(?:not supported|unsupported|not allowed|not recognized)`, 'i').test(message);
}

function explicitlyRejectsTools(message: string): boolean {
  // A provider rejecting a tool's schema has not rejected tool calling itself.
  if (/\$ref|\bschema\b|tools\s*\[/i.test(message)) return false;
  if (['tools', 'tool_choice', 'functions', 'function_call'].some(parameter => explicitlyRejectsParameter(message, parameter))) return true;
  return /\b(?:tools?|tool[_ ]?calling|tool_choice|function[_ ]?call(?:ing)?)\b[\s\S]{0,50}(?:not supported|unsupported|not available)|(?:unsupported|not support(?:ed)?|not implement(?:ed)?)[\s\S]{0,50}\b(?:tools?|tool[_ ]?calling|tool_choice|function[_ ]?call(?:ing)?)\b/i.test(message);
}
...
export function classifyWorkshopError(error: unknown): WorkshopAgentError {
  ...
  if ((status === 400 || status === 422 || status === 501) && explicitlyRejectsTools(message)) {
...
export function createWorkshopModel(config: ApiConfig, fetchImpl: typeof nativeFetch = nativeFetch, options: { textOnly?: boolean } = {}) {
  ...
  const rejectedParameters = new Set<string>();
  const provider = createOpenAICompatible({
    name: 'workshop', baseURL, apiKey: config.apiKey || undefined, supportsStructuredOutputs: false,
    transformRequestBody: body => {
      const request: Record<string, unknown> = {
        ...body,
        ...(Array.isArray(body.tools) && body.tools.length > 0 ? { parallel_tool_calls: false } : {}),
      ...
      if (options.textOnly) {
        for (const parameter of ['tools', 'tool_choice', 'parallel_tool_calls', 'response_format']) delete request[parameter];
      }
      for (const parameter of rejectedParameters) delete request[parameter];
      return request;
    },
    fetch: Object.assign(async (_url: RequestInfo | URL, init?: RequestInit) => {
      ...
      // Only retry rejected requests, never a successful generation/tool execution.
      // Each retry removes a distinct optional field, so there are at most three.
      for (;;) {
        init?.signal?.throwIfAborted();
        response = await fetchImpl(request.url, { ...init, body: requestBody, headers: request.headers });
        init?.signal?.throwIfAborted();
        if ((response.status !== 400 && response.status !== 422) || typeof requestBody !== 'string') break;
        const errorText = await response.clone().text();
        let message = errorText;
        try {
          const error = JSON.parse(errorText);
          message = typeof error?.error?.message === 'string' ? error.error.message : typeof error?.message === 'string' ? error.message : errorText;
        } catch { /* Some compatible services return plain-text errors. */ }
        const body = JSON.parse(requestBody) as Record<string, unknown>;
        const rejected = optionalParameters.find(parameter => Object.hasOwn(body, parameter) && explicitlyRejectsParameter(message, parameter));
        if (!rejected) break;
        rejectedParameters.add(rejected);
