    ...
    return {
      ...
      importModule: async raw => {
        ...
        if (previous && previous.id !== currentWorkshopDraft(next)?.id) {
          get().stop();
          const currentScope = ++scope;
          // Persist the new session and its imported revision together. A later
          // selection must never redirect this import into another session.
          const imported = await enqueue(() => insertWorkshopSession(
            commitWorkshopRevision(createWorkshopSession(session.world), value, 0, '导入模块'),
          ));
          cache.set(imported.id, imported);
          if (scope === currentScope) { show(imported); set({ activeSessionId: imported.id, loading: false, error: undefined }); }
          return;
        }
        get().stop();
