...
const decisionSchema = z.object({
  ...
  plans: z.array(z.object({ id: nonempty, intent: nonempty, participants: z.array(nonempty).max(12), constraints: z.array(nonempty).max(12), priority: z.number().int().min(0).max(70), visibility: z.enum(['foreground', 'reader_only', 'secret']), evidence: z.array(evidence).min(1).max(8) }).strict()).max(6),
...
export async function requestDirectorDecision(state: DirectorState, context: DirectorReadContext, worldDescription: string, config: ApiConfig, signal?: AbortSignal): Promise<DirectorDecision> {
  ...
  const result = await requestCompletion(config, [{ role: 'system', content: DIRECTOR_INSTRUCTIONS }, { role: 'user', content: JSON.stringify(input) }], { temperature: 0.3, maxTokens: 6000, responseFormat: 'json', signal });
  return decisionSchema.parse(JSON.parse(result.text.replace(/^\s*```(?:json)?\s*/i, '').replace(/\s*```\s*$/, '')));
}
