...
资源变化必须提交 GameplayTransaction JSON（通过现有更新标签传输），例如：
{"id":"ai:survival:gather","source":"ai","moduleId":"survival","effects":[{"add":{"path":"玩家.生存资源.food.数量","delta":3,"min":0}}]}

规则：
- 只提交发生变化的资源，未变化的不要输出
- 数量不能为负数；资源消耗优先用 add 的负 delta
- 多个资源同时变化时放在同一个事务中
- 蓝图内资源由演化系统处理，AI 不得重复创建或覆盖资源元数据
- 禁止直接输出玩家状态对象或 RFC 补丁；所有消耗、奖励和事件必须在同一事务中表达
