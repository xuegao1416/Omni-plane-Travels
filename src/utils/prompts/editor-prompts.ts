...
【创建新NPC - 必须包含以下全部字段】
创建时使用 set effect，路径为 人物档案.角色名，value 为完整 NPC 对象。value 必须包含姓名、种族、性别、年龄、人物分类、社会身份（职业与社会地位都要填）、关系数据、个人信息、人物事迹、当前行动、短期目标和长期目标；并按角色身份填入 技能列表（1~3 项）与 物品列表（随身物品，确实一无所有时填空对象）、装备列表；启用数值/成长模块时再填入对应模块字段。这些字段即使玩家当下看不全也必须生成，系统只对玩家侧做隐藏。
