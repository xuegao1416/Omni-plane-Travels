...
/**
 * SillyTavern 数值 position：0=before, 1=after, 4=atDepth, 2~6=AN/EM。
 * 本应用支持 before_char / after_char / at_depth 三个槽位；
 * atDepth 需要 depth 字段配合（注入聊天历史指定深度），其余（2/3/5/6）落到 after_char。
 */
function normalizePosition(value: unknown): 'before_char' | 'after_char' | 'at_depth' {
  if (typeof value === 'string') {
    const normalized = normalizeKeyName(value);
    if (normalized === 'before_char' || normalized === 'beforechar' || normalized === 'before') {
      return 'before_char';
    }
    if (normalized === 'at_depth' || normalized === 'atdepth') {
      return 'at_depth';
    }
    return 'after_char';
  }
  const numeric = toFiniteNumber(value);
  if (numeric === undefined) return 'after_char';
  if (numeric === 0) return 'before_char';
  if (numeric === 4) return 'at_depth';
  return 'after_char';
}
...
function normalizeWorldBookEntry(
  ...
  // 角色卡 v2/v3：真实 ST position/depth 藏在 extensions 里（顶层 position 只有 spec 的 before/after 字符串）
  const extensions = isPlainObject(item.extensions) ? item.extensions : null;
  const extPosition = extensions ? extensions.position ?? extensions.insertion_position : undefined;
  const extDepth = extensions ? extensions.depth : undefined;
  const rawPosition = item.position ?? item.insertion_position ?? item.insertionPosition ?? extPosition;
  let position = normalizePosition(rawPosition);
  if (position !== 'at_depth' && extPosition !== undefined && normalizePosition(extPosition) === 'at_depth') {
    position = 'at_depth';
  }
  ...
  const depth = toFiniteNumber(item.depth ?? extDepth);
  ...
  if (isPlainObject(origin) && typeof origin.datasetId === 'string'
    && typeof origin.sourceKey === 'string' && typeof origin.generatedHash === 'string'
    && typeof origin.analysisVersion === 'number' && Number.isFinite(origin.analysisVersion)) {
    entry.novelProvenance = origin as unknown as WorldBookEntryDef['novelProvenance'];
  }
